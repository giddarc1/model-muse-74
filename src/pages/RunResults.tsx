...
import { Play, CheckCircle, AlertTriangle, Shield, XCircle } from 'lucide-react';
...
export default function RunResults() {
  ...
  const handleRun = () => {
    ...
    setTimeout(() => {
      const calcResults = calculate(model, activeScenario || null);
  ...
  return (
    <div className="p-6 animate-fade-in">
      ...
      <p className="text-sm text-muted-foreground mb-1">
        ...
        {activeScenario && (
          <Badge className="ml-2 bg-warning/20 text-warning text-[10px] border-0">
          ...
          <TabsContent value="summary" className="mt-4">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Consolidated production metrics ({activeScenario ? activeScenario.name : 'Basecase'})</CardDescription>
              </CardHeader>
              <CardContent className="p-0 overflow-x-auto">
                <Table>
                  <TableHeader>
                  ...
                  <TableBody>
                    {results!.products.map(row => (
                      <TableRow key={row.id}>
                        ...
                        <TableCell className="font-mono text-right font-medium">{row.mct.toFixed(2)}</TableCell>
                      </TableRow>
                    ))}
